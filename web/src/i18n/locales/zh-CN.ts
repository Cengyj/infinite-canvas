export default {
    grokVideo: { mediaReferenceUnsupported: "Grok 视频暂不支持参考视频或音频，请移除后重试。", durationUnsupported: "Grok 视频时长必须为 1 至 15 秒", aspectRatioUnsupported: "Grok 视频画幅仅支持 1:1、16:9、9:16、4:3、3:4、3:2 或 2:3", resolutionUnsupported: "Grok 视频清晰度仅支持 480p、720p 或 1080p", resolutionCombinationUnsupported: "Grok 1080p 仅支持 1.5 模型且最多使用一张参考图" },
    meta: {
        title: "无限画布",
    ...
    modelPlugin: {
        ...
        variables: { prompt: "用户输入的提示词（已拼接系统提示词）", images: "参考图地址数组（dataURL 或 URL；改图、图生视频及多模态文本请求时有值）", videos: "参考视频 File 数组，可直接 form.append；画布或工作台接入的参考视频，无则为空数组", audios: "参考音频 File 数组，可直接 form.append；画布或工作台接入的参考音频，无则为空数组", messages: "对话消息数组，含系统消息", params: "生成参数：生图 {size,requestedSize,quality,count,background}（requestedSize 仅 Gemini 为用户原始尺寸）、视频 {mode,seconds,size,resolution,ratio,generateAudio,watermark}（mode 为 frames 首尾帧或 reference 全能参考；超过 2 张图时为 reference）、音频 {voice,format,speed,instructions}", model: "模型名称（不含渠道前缀）", baseUrl: "渠道接口地址（原样，未拼 /v1）", apiKey: "渠道 API Key，请求头里自己带上", systemPrompt: "系统提示词原文", reasoningEffort: "文本推理强度；auto 表示由脚本决定是否传递", http: "便捷请求：http.post(path, body, {headers,params,responseType})、http.get(path, opts)、http.url(path)；默认带 Authorization: Bearer apiKey，可用 headers 覆盖；普通相对路径按 baseUrl 拼 /v1，显式 /v1 或 /v1beta 路径保持所选版本", request: "原始请求 request({ method, url, headers, params, data, responseType })，不加任何默认头，鉴权头自己写；相对 URL 与 http 使用相同的版本路径规则，也可传完整 URL", poll: "轮询 poll(request, extract, {intervalMs,timeoutMs})，extract 返回真值即结束", sleep: "sleep(ms) 延时", signal: "取消信号，可透传给 http/request", onDelta: "onDelta(text) 推送流式文本（文本模型）", fetch: "代理感知的 fetch；HTTP(S) 地址会按当前本地代理设置转发，并将跨域或代理网络错误转换为可读提示" },
        ...
        templates: {
            openaiStandard: "OpenAI Sora（标准字段）",
            geminiVeo3ResolutionUnsupported: "Veo 3 视频模板当前仅支持 720p",
            geminiVeo3SecondsUnsupported: "Veo 3 视频仅支持 4、6 或 8 秒",
            geminiVeo2SecondsUnsupported: "Veo 2 视频仅支持 5、6 或 8 秒",
            geminiVideoSquareUnsupported: "Gemini Veo 视频不支持方形画幅，请选择横屏或竖屏",
            videoSingleReference: "Gemini 视频模板最多支持 1 张参考图",
            openaiVideoSizeUnsupported: "OpenAI 视频尺寸仅支持 720x1280、1280x720、1024x1792 或 1792x1024",
            openaiVideoSecondsUnsupported: "OpenAI 视频仅支持 4、8 或 12 秒",
            openaiVideoSingleReference: "OpenAI 视频模板最多支持 1 张参考图", openai: "OpenAI 规范", gemini: "Gemini 规范" },
        authoring: {
    ...
    apiErrors: {
        promptOptimizationReferencePayload: "参考图处理后仍超过提示词优化载荷限制，请减少或压缩图片，或关闭参考图分析",
        promptOptimizationReferenceLimit: "提示词优化最多分析 {{count}} 张参考图，请减少图片数量或关闭参考图分析",
        videoReferenceLimit: "视频生成最多支持 {{count}} 张参考图，请减少图片数量",
        textResponseIncomplete: "文本模型未完整生成结果（{{reason}}），请重试", corsRequired: "请求失败，请检查网络连接、接口地址或代理设置", requestFailed: "请求失败", requestCanceled: "请求已取消", baseUrlRequired: "请先配置 Base URL", apiKeyRequired: "请先配置 API Key", authenticationFailed: "鉴权失败，请检查 API Key、套餐权限或模型权限", rateLimited: "请求被限流或额度不足，请稍后重试", notFound: "接口地址不存在（404），请检查 Base URL 和模型选择", badGateway: "网关错误（502），接口服务暂时不可用，请稍后重试", serviceBusy: "服务繁忙（503），请稍后重试", httpFailed: "请求失败（HTTP {{status}}），请检查 Base URL 和 API Key 是否正确", htmlError: "服务返回了 HTML 错误页面（{{preview}}）", imageTimeout: "生图请求超时，接口长时间没有返回，请稍后重试或更换渠道", audioModelRequired: "请先配置音频模型", audioGenerationFailed: "音频生成失败", scriptNoAudio: "模型调用脚本没有返回音频", geminiAudioUnsupported: "Gemini 调用格式暂不支持音频生成，请使用 OpenAI 格式渠道", invalidImageSizeFormat: "图像尺寸格式不支持，请使用 auto、9:16 或 1024x1024", positiveImageRatio: "图像比例必须是正数，例如 9:16", imageRatioLimit: "图像宽高比不能超过 3:1，请调整尺寸", positiveImageDimensions: "图像尺寸必须是正整数，例如 1024x1024", imageDimensionStep: "图像尺寸的宽高必须是 16 的倍数，请调整尺寸", imageEdgeLimit: "图像尺寸最长边不能超过 3840px，请调整尺寸", imagePixelLimit: "图像总像素需在 655360 到 8294400 之间，请调整尺寸", unknownImageResponse: "接口返回了未知格式的数据（字段：{{fields}}），请检查模型或接口兼容性", noImageReturned: "接口没有返回图片，请检查提示词是否触发安全审核或模型是否支持该操作", geminiImageSizeUnsupported: "Gemini 模型 {{model}} 不支持 {{size}} 图片输出尺寸，支持的尺寸：{{supported}}", geminiMaskUnsupported: "Gemini 调用格式暂不支持蒙版编辑", geminiRejected: "Gemini 拒绝了本次请求：{{reason}}", geminiNoImage: "Gemini 接口没有返回图片", maskModelUnsupported: "蒙版编辑暂不支持该模型，请使用其他渠道", noContent: "没有返回内容", modelReadFailed: "读取模型失败", videoTimeout: "{{provider}}视频生成超时，请稍后重试", pluginVideoExpired: "插件视频任务已失效，请重新生成", scriptNoVideo: "模型调用脚本没有返回视频", noPlayableVideo: "视频接口没有返回可播放的视频", noVideoTaskId: "视频接口没有返回任务 ID", videoTaskCreateFailed: "视频任务创建失败", videoGenerationFailed: "视频生成失败", videoTaskQueryFailed: "视频任务查询失败", videoPromptRequired: "请输入视频提示词，或连接参考图片/视频/音频", referenceImageReadFailed: "参考图读取失败，请换一张图片或重新上传", invalidReferenceVideo: "参考视频必须是公网 URL、资产 ID，或本地已保存的视频", invalidReferenceAudio: "参考音频必须是公网 URL、资产 ID，或本地已保存的音频", videoModelRequired: "请先配置视频模型", geminiVideoUnsupported: "Gemini 调用格式暂不支持视频生成，请使用 OpenAI 格式渠道", noVideoTask: "接口没有返回视频任务", videoDownloadFailed: "视频下载失败", localAssetReadFailed: "读取本地资产失败" },
    prompts: {
        title: "提示词中心",
        library: "提示词库",
    ...
    imageWorkbench: {
        promptOptimization: {
            action: "优化提示词",
            title: "优化生图提示词",
            description: "由文本模型补全画面表达，优化结果确认后才会回填，不会自动开始生图。",
            model: "优化模型",
            referenceCount: "当前有 {{count}} 张参考图；为保护隐私，首次打开时默认不分析图片内容。",
            noReferences: "当前没有参考图；优化请求只发送提示词及生成上下文。",
            analyzeReferences: "分析参考图",
            referenceLimit: "一次最多分析 {{count}} 张参考图，请减少图片数量或保持关闭参考图分析。",
            analyzeReferencesHint: "开启后，最多 {{count}} 张参考图会压缩并发送给当前文本模型，由模型结合可见内容优化提示词。",
            referencesTextOnly: "开关关闭时不会发送图片内容，仅发送 {{count}} 张参考图的编号、数量及当前生成上下文。",
            source: "当前提示词",
            sourceHint: "可在这里微调",
            sourcePlaceholder: "输入需要优化的生图描述",
            requirements: "补充要求",
            requirementsHint: "可选",
            requirementsPlaceholder: "例如：保留留白、改为俯拍、避免出现文字",
            requirementsReferencePlaceholder: "例如：图片1保留主体，图片2仅参考服装或风格",
            result: "优化结果",
            resultHint: "回填前可编辑",
            resultPlaceholder: "优化完成后将在这里显示结果",
            optimizing: "正在生成优化结果…",
            optimize: "开始优化",
            reoptimize: "重新优化",
            continue: "用结果继续优化",
            ...
            apply: "应用到提示词",
            applied: "已应用优化后的提示词",
            referenceChanged: "参考图已变化，请检查提示词中的图片编号后再次生成",
            contextChanged: "画面尺寸或透明背景设置已变化，请确认优化结果仍适用后再次生成",
            referenceAndContextChanged: "参考图及画面设置均已变化，请检查图片编号并确认优化结果仍适用后再次生成",
            configRequired: "请先配置可用的文本模型和渠道凭据",
            failed: "提示词优化失败",
            status: { waiting: "等待优化", running: "优化中", ready: "可应用", stale: "需再次优化" },
        },
        editReferenceLimit: "OpenAI 格式图片编辑最多支持 {{count}} 张参考图，请减少图片数量",
        promptActions: { prompts: "提示词库", assets: "查看资产", optimize: "优化提示词" },
        title: "生图工作台",
    ...
    videoWorkbench: {
        promptOptimization: {
            action: "优化提示词",
            title: "优化视频提示词",
            description: "由文本模型补全动作、镜头与时序表达，优化结果确认后才会回填，不会自动开始视频生成。",
            model: "优化模型",
            referenceCount: "当前有 {{count}} 张参考图；为保护隐私，首次打开时默认不分析图片内容。",
            noReferences: "当前没有参考图；优化请求只发送提示词及生成上下文。",
            analyzeReferences: "分析参考图",
            referenceLimit: "一次最多分析 {{count}} 张参考图，请减少图片数量或保持关闭参考图分析。",
            analyzeReferencesHint: "开启后，最多 {{count}} 张参考图会压缩并发送给当前文本模型，用于保持主体、场景和风格连续性。",
            referencesTextOnly: "开关关闭时不会发送图片内容，仅发送 {{count}} 张参考图的编号、数量及当前生成上下文。",
            source: "当前提示词",
            sourceHint: "可在这里微调",
            sourcePlaceholder: "输入需要优化的视频描述",
            requirements: "补充要求",
            requirementsHint: "可选",
            requirementsPlaceholder: "例如：镜头缓慢推进、保持主体一致、结尾回到全景",
            requirementsReferencePlaceholder: "例如：图片1作为主体参考，图片2仅参考场景，镜头缓慢推进",
            result: "优化结果",
            resultHint: "回填前可编辑",
            resultPlaceholder: "优化完成后将在这里显示结果",
            optimizing: "正在生成优化结果…",
            ...
            stop: "停止优化",
            apply: "应用到提示词",
            applied: "已应用优化后的视频提示词",
            referenceChanged: "参考图已变化，请检查提示词中的图片编号后再次生成",
            contextChanged: "视频时长、画面尺寸或声音设置已变化，请确认优化结果仍适用后再次生成",
            referenceAndContextChanged: "参考图及视频设置均已变化，请检查图片编号并确认优化结果仍适用后再次生成",
            configRequired: "请先配置可用的文本模型和渠道凭据",
            failed: "提示词优化失败",
            status: { waiting: "等待优化", running: "优化中", ready: "可应用", stale: "需再次优化" },
        },
        ...
        removeAudio: "移除参考音频",
        dropReferences: "松开即可添加参考图",
        noImages: "暂无参考图，可拖入图片，最多 {{count}} 张",
        noImagesUnlimited: "暂无参考图，可将图片拖到这里",
        noVideos: "暂无参考视频，可拖入文件，最多 3 个",
        noAudio: "暂无参考音频，可拖入文件，最多 3 个，mp3/wav，单个 15MB 内",
        unsupportedFiles: "已忽略非图片文件，视频工作台参考输入仅支持图片",
    ...
    config: {
        ...
        proxy: {
            title: "本地代理",
            description: "可选转发工具。网站与 API 域名不同也可直接请求，无需因此开启代理；直连被浏览器跨域策略拦截时可按需开启。开启后，模型请求、资源下载和 WebDAV 同步会通过本机代理转发。",
            startHint: "先在终端运行下面的命令，并在使用画布期间保持运行：",
            originHint: "代理默认允许合法的 HTTP(S) 页面来源；如需收紧到指定页面，可手动追加 --allow-origin <页面 Origin>，多个来源可重复追加。",
            address: "代理地址",
            ...
            originNotAllowed: "本地代理拒绝了当前页面来源；HTTP(S) 页面请移除过窄的 --allow-origin 限制或加入当前 Origin，file:// 页面请改用 HTTP(S) 地址。",
        },
        localStorage: {
        ...
        modelSelect: {
            ...
            add: "增加模型",
            fetch: "拉取模型列表",
            description: "如果上游不提供模型列表接口，请在这里手动增加模型名称。",
        ...
        webdav: {
            localReadFailed: "本地数据未完整读取，已暂停同步以保护现有文件；请刷新后重试",
            title: "WebDAV 同步",
            ...
            errors: { testFailed: "WebDAV 连接测试失败", downloadFailed: "读取 WebDAV 同步文件失败", downloadTimeout: "读取 WebDAV 同步文件超时", emptyUpload: "上传文件为空，已取消上传", uploadFailed: "上传 WebDAV 同步文件失败", directoryFailed: "创建 WebDAV 远程目录失败", requestTimeout: "WebDAV 请求超时，请检查网络或远端服务状态", connectionFailed: "无法连接 WebDAV，请检查地址、HTTPS 证书或网络状态", urlRequired: "请先填写 WebDAV 地址", authenticationFailed: "WebDAV 认证失败，请检查用户名、密码或应用密码", pathMissing: "WebDAV 路径不存在，请检查地址和远程目录", responseFailed: "{{fallback}}：{{status}}{{detail}}", syncFailed: "同步失败", invalidManifest: "{{domain}} 同步清单不是当前应用的数据" },
        },
        protocols: {
        },
    },
