export default {
    ...
    theme: { toggle: "切换主题" },
    grokVideo: { durationUnsupported: "Grok 视频时长必须为 1 至 15 秒", aspectRatioUnsupported: "Grok 视频画幅仅支持 1:1、16:9、9:16、4:3、3:4、3:2 或 2:3", resolutionUnsupported: "Grok 视频清晰度仅支持 480p、720p 或 1080p", resolutionCombinationUnsupported: "Grok 1080p 仅支持 1.5 模型且最多使用一张参考图" },
    common: {
        cancel: "取消",
        save: "保存",
        edit: "编辑",
    ...
    settingsPanels: {
        ...
        video: { title: "视频设置", quality: "清晰度", size: "尺寸", seconds: "秒数", resolution: "清晰度档位", ratio: "画幅比例", duration: "时长", output: "输出", generateAudio: "生成声音", watermark: "添加水印", adaptive: "自适应", sizes: { landscape: "横屏", portrait: "竖屏", square: "方形", widescreen: "宽屏", tall: "长图", auto: "自动" }, ratios: { landscape: "横屏", portrait: "竖屏", square: "方形", standardLandscape: "标准横屏", standardPortrait: "标准竖屏", cinematic: "宽银幕", adaptive: "自适应" } },
    ...
    modelPlugin: {
        ...
        templates: { openai: "OpenAI 规范", gemini: "Gemini 规范", imageOpenai: "生图 / 改图：两者接口不同，用 images 是否为空来区分。", availableImage: "可用：prompt、images(dataURL[])、params{size,quality,count}、model、baseUrl、apiKey", textToImage: "文生图：/images/generations（JSON）", imageToImage: "图生图：/images/edits（multipart/form-data，参考图作为文件上传）", formDataHeader: "不要手动设 Content-Type，交给浏览器带 boundary", imageGemini: "Gemini 文生图 / 图生图：都走 generateContent，参考图放进 parts 的 inlineData。", availableImageGemini: "可用：prompt、images(dataURL[])、model、baseUrl、apiKey", videoOpenai: "OpenAI 视频：multipart 创建任务并轮询，支持至多 1 张 input_reference 参考图；完成后使用直连地址或下载 /content。可用：prompt、images(dataURL[])、params{seconds,size,resolution,ratio}", openaiVideoSingleReference: "OpenAI 视频模板最多支持 1 张参考图", openaiVideoSecondsUnsupported: "OpenAI 视频仅支持 4、8 或 12 秒", openaiVideoSizeUnsupported: "OpenAI 视频尺寸仅支持 720x1280、1280x720、1024x1792 或 1792x1024", videoGemini: "Gemini(Veo) 视频：predictLongRunning 提交并轮询 operation，最多使用 1 张首帧参考图；完成后使用 API Key 请求头下载视频。", availableVideoGemini: "可用：prompt、images(dataURL[])、params、model、baseUrl、apiKey", videoSingleReference: "Gemini 视频模板最多支持 1 张参考图", geminiVideoSquareUnsupported: "Gemini Veo 视频不支持方形画幅，请选择横屏或竖屏", geminiVeo2SecondsUnsupported: "Veo 2 视频仅支持 5、6 或 8 秒", geminiVeo3SecondsUnsupported: "Veo 3 视频仅支持 4、6 或 8 秒", geminiVeo3ResolutionUnsupported: "Veo 3 视频模板当前仅支持 720p", geminiNoVideoUri: "Gemini 未返回视频 URI", audioOpenai: "音频 TTS。可用：prompt、params{voice,format,speed,instructions}、model", audioGemini: "Gemini TTS：generateContent + AUDIO 模态，返回 base64 PCM（音频数据在 inlineData.data）。", availableAudioGemini: "可用：prompt、params{voice}、model、baseUrl、apiKey", geminiNoAudio: "Gemini 未返回音频", textOpenai: "文本对话（OpenAI Responses 接口）。messages.content 可能包含 text / image_url，多模态内容需转换为 input_text / input_image。", textGemini: "Gemini 文本：system 消息放 systemInstruction，多模态 image_url 转为 inlineData 或 fileData。", availableTextGemini: "可用：messages([{role,content}])、images(dataURL或URL[])、systemPrompt、model、baseUrl、apiKey" },
    },
    apiErrors: { requestFailed: "请求失败", requestCanceled: "请求已取消", corsRequired: "请求被浏览器跨域策略拦截，请通过自己的服务转发请求", baseUrlRequired: "请先配置 Base URL", apiKeyRequired: "请先配置 API Key", authenticationFailed: "鉴权失败，请检查 API Key、套餐权限或模型权限", rateLimited: "请求被限流或额度不足，请稍后重试", notFound: "接口地址不存在（404），请检查 Base URL 和模型选择", badGateway: "网关错误（502），接口服务暂时不可用，请稍后重试", serviceBusy: "服务繁忙（503），请稍后重试", httpFailed: "请求失败（HTTP {{status}}），请检查 Base URL 和 API Key 是否正确", htmlError: "服务返回了 HTML 错误页面（{{preview}}）", audioModelRequired: "请先配置音频模型", audioGenerationFailed: "音频生成失败", scriptNoAudio: "模型调用脚本没有返回音频", geminiAudioUnsupported: "Gemini 调用格式暂不支持音频生成，请使用 OpenAI 格式渠道", invalidImageSizeFormat: "图像尺寸格式不支持，请使用 auto、9:16 或 1024x1024", positiveImageRatio: "图像比例必须是正数，例如 9:16", imageRatioLimit: "图像宽高比不能超过 3:1，请调整尺寸", positiveImageDimensions: "图像尺寸必须是正整数，例如 1024x1024", imageDimensionStep: "图像尺寸的宽高必须是 16 的倍数，请调整尺寸", imageEdgeLimit: "图像尺寸最长边不能超过 3840px，请调整尺寸", imagePixelLimit: "图像总像素需在 655360 到 8294400 之间，请调整尺寸", unknownImageResponse: "接口返回了未知格式的数据（字段：{{fields}}），请检查模型或接口兼容性", noImageReturned: "接口没有返回图片，请检查提示词是否触发安全审核或模型是否支持该操作", geminiRejected: "Gemini 拒绝了本次请求：{{reason}}", geminiImageSizeUnsupported: "Gemini 模型 {{model}} 不支持 {{size}} 图片输出尺寸，支持的尺寸：{{supported}}", geminiNoImage: "Gemini 接口没有返回图片", geminiMaskUnsupported: "Gemini 调用格式暂不支持蒙版编辑", maskModelUnsupported: "蒙版编辑暂不支持该模型，请使用其他渠道", noContent: "没有返回内容", textResponseIncomplete: "文本模型未完整生成结果（{{reason}}），请重试", modelReadFailed: "读取模型失败", videoTimeout: "{{provider}}视频生成超时，请稍后重试", pluginVideoExpired: "插件视频任务已失效，请重新生成", scriptNoVideo: "模型调用脚本没有返回视频", noPlayableVideo: "视频接口没有返回可播放的视频", noVideoTaskId: "视频接口没有返回任务 ID", videoTaskCreateFailed: "视频任务创建失败", videoGenerationFailed: "视频生成失败", videoTaskQueryFailed: "视频任务查询失败", videoPromptRequired: "请输入视频提示词，或连接参考图片/视频/音频", videoReferenceLimit: "视频生成最多支持 {{count}} 张参考图，请减少图片数量", referenceImageReadFailed: "参考图读取失败，请换一张图片或重新上传", promptOptimizationReferenceLimit: "提示词优化最多分析 {{count}} 张参考图，请减少图片数量或关闭参考图分析", promptOptimizationReferencePayload: "参考图处理后仍超过提示词优化载荷限制，请减少或压缩图片，或关闭参考图分析", invalidReferenceVideo: "参考视频必须是公网 URL、资产 ID，或本地已保存的视频", invalidReferenceAudio: "参考音频必须是公网 URL、资产 ID，或本地已保存的音频", videoModelRequired: "请先配置视频模型", geminiVideoUnsupported: "Gemini 调用格式暂不支持视频生成，请使用 OpenAI 格式渠道", noVideoTask: "接口没有返回视频任务", videoDownloadFailed: "视频下载失败", localAssetReadFailed: "读取本地资产失败" },
    prompts: {
        title: "提示词中心",
        library: "提示词库",
