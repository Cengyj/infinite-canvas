...
启动后会生成 `Connect token`，网页连接时必须携带 token。由于原生 EventSource 不能设置请求头，浏览器建立 SSE 长连接时暂时仍把 token 放在 query 中：

```txt
GET /events?token=xxx&clientId=xxx（EventSource 暂以 query token 认证）
```
