...
+ [新增] 恢复图片、视频工作台和画布中的提示词优化，支持参考图分析、生成上下文、流式预览、取消恢复和自定义文本渠道。
+ [新增] 增加 Grok 视频专用 JSON 参数、1–15 秒、比例与参考图校验，并保留普通视频模型的首尾帧、多图及音视频参考能力。
+ [优化] 视频轮询统一为 3 秒间隔、20 分钟期限；资产包导入、WebDAV 同步和未保存媒体引用增加生命周期保护，避免并发清理误删。
+ [修复] 文本流只有收到完整终态才会回填结果，连接中断、拒绝、截断和上游错误不再伪装成成功；新环境默认文本模型改为 `gpt-6-sol`。

+ [修复] 切换渠道协议时保留当前 Base URL，避免预置或自定义接口地址被替换为官方地址。
+ [修复] 渠道地址导入只规范化路径，保留查询参数，避免不同参数的渠道被错误合并或覆盖凭据。
+ [修复] Veo 轮询和内置模板的相对资源地址按 API 提供方解析，避免跟随网站域名或协议请求错误目标。
+ [修复] 代理来源拒绝只依据专用诊断标记，避免误判上游 403 或等待其错误正文，同时修复分块上传并明确异域 API 可直接调用。
+ [修复] 完善画布项目切换与 Agent 上下文生命周期，避免普通画布操作误阻止生图导入、迟到结果跨项目写入和异步消息串入其他线程。
+ [修复] 媒体清理保留生成来源、撤销历史、剪贴板和工作台临时参考，并复核并发写入与排队删除期间的引用变化。
+ [修复] 本地代理保留目标 URL 编码与签名参数，来源拒绝诊断仅用于实际代理请求，代理连通性与版本检查增加超时和明确错误提示。
+ [修复] Gemini 流式请求保留 Base URL 查询参数，旧配置恢复不再注入默认模型，Veo 内置脚本不再将认证、取消或代理下载失败误记为成功。
+ [修复] WebDAV 同步拒绝未知版本或缺少主要结构的远端清单，避免将无效数据当作空内容覆盖。
+ [修复] Canvas Agent 生图结果按任务发起时的画布项目与项目修订绑定，避免下载前后切换项目时将图片误写到其他画布。
+ [修复] Canvas Agent 可导入 Windows 盘符和 UNC 路径中的生成图片，仅按页面、线程、轮次和条目短时授权当前 Codex 生图事件返回的路径，限制单次数量与总大小，并避免单张读取失败阻断其他结果。
+ [修复] Gemini 生图调用脚本与原生请求统一模型尺寸校验、4K 方图识别和 `inlineData` 参考图结构，同时保持已有脚本的 `params.size` 提交语义，并恢复模型列表说明的协议中立表述。
+ [修复] 模型渠道持久化增加损坏数据容错，旧版扁平配置可正确转换为单渠道；带查询参数的 Base URL 可正确拼接 OpenAI 与 Gemini 路径，GPT Image 与 DALL-E 不再收到彼此不兼容的参数。
+ [修复] Gemini Veo 的图片、视频和音频参考改用标准 `inlineData` 结构，同渠道认证下载失败时不再把无凭据地址误记为成功结果。
+ [修复] 快速切换画布项目时隔离异步恢复与持久化，避免旧项目迟到数据覆盖或写入新项目；撤销历史淘汰和切换项目后会重新清理无引用媒体。
+ [修复] WebDAV 为素材、生图历史和视频历史同步删除记录，并在同步期间增量合并本地变更，避免已删除内容复活、视频成功状态倒退或并发写入丢失。
+ [修复] 删除生成历史改为按全局引用清理媒体，素材和其他历史仍引用的文件不会误删；删除任务或离开视频页会取消轮询，资产包导入会重建本机存储键与预览地址。
+ [修复] 图片缩略图替换、媒体写入与删除增加并发版本和写入隔离，失败后可重新生成；视频历史文件和插件临时结果按实际引用保留或释放，避免旧缩略图、历史失效和长期内存占用。
+ [修复] Canvas Agent 画布工具调用按线程、轮次、页面和项目隔离，异步附件落入画布前会再次校验项目；重复或超长消息 ID 不再覆盖已有元数据与预览资源。
+ [调整] 本地代理默认接受任意合法 HTTP(S) 页面来源，避免托管页面因来源变化被拦截；仍可用 `--allow-origin` 精确收紧，签名 URL 日志会脱敏，浏览器断开或上游响应流异常时会主动中止并清理流。
+ [修复] 启用本地代理时官方插件注册表、插件源码、提示词源、版本检查和自定义脚本 `fetch` 统一经过代理，并显示代理不可用或来源未授权的具体原因。
+ [优化] 本地代理配置不再为托管页面自动追加单一 Origin，支持多个可信页面直接使用，并区分来源拒绝、代理不可用与普通网络失败，避免在没有浏览器证据时误报跨域。
+ [修复] 未启用本地代理时，跨源图片即使无法读取响应字节但仍能由浏览器显示，也会保留原图地址，不再因普通网络错误被错误阻断。
+ [修复] 合并 v0.17.0–v0.19.0 的画布组、视频任务、缩略图、本地代理和 ZCode 等功能，并修复视频远程资源下载绕过本地代理的问题。
+ [修复] Canvas Agent 配置、消息元数据与事件历史改为版本严格校验、原子写入和损坏隔离，拒绝覆盖未知或损坏的本地数据。
+ [修复] Canvas Agent 重放事件按 `threadId`、`turnId` 和 `itemId` 隔离，避免跨线程覆盖或重复显示；消息元数据读取失败时不再静默展示不完整历史。
+ [优化] Canvas Agent 普通 HTTP 请求使用 Header Token，CLI stderr、错误和工具日志统一脱敏或摘要化，降低凭据与提示词泄露风险。
+ [修复] OpenAI 图片调用脚本模板与 GPT Image 参数行为对齐，自动尺寸字段不再写入 `undefined`。
+ [修复] Gemini 视频调用脚本模板校验任务来源并通过 Header 下载结果，避免把 API Key 拼入视频 URL。
+ [修复] 原生 Gemini 视频任务轮询改用同源请求头下载结果，并拒绝跨域 operation URL，避免凭据进入查询参数。
+ [修复] 远程资产下载在启用本地代理时统一经过代理转发，减少跨域下载失败。
+ [修复] 视频节点截帧读取远程媒体增加 30 秒超时并清理监听，代理或上游无响应时不会永久挂起。
...
+ [修复] 修复 Gemini 原生图片生成请求误用 `responseFormat.image`，改为通过 `imageConfig` 传递宽高比与输出尺寸。
+ [修复] 修复无本地配置时持久化合并误清空预置渠道，导致新安装仅显示一个渠道。
+ [优化] Gemini 原生生图按模型能力提交宽高比与输出尺寸，并在已知模型不支持所选尺寸时提前提示。
+ [优化] 统一配置持久化、文件导入和渠道整体更新的规范化与版本管理，避免重复写入及导入后状态不一致。
+ [调整] 新安装默认预置 OpenAI 与 Google 两个模型渠道并统一使用 `https://direct.foropencode.com`，Google 仅展示三个 Gemini Preview 生图模型。
...
+ [修复] 修复 Gemini 原生图片生成请求误用 `responseFormat.image`，改为通过 `imageConfig` 传递宽高比与输出尺寸。
+ [修复] 按 xAI 官方规范提交 `grok-imagine-image` 文生图参数，并按响应 MIME 解析和下载生成图片。
