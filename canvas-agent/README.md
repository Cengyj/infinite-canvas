...
Canvas Agent 默认只监听 `127.0.0.1`。每个 HTTP(S) 网页带正确 token 连接后，Canvas Agent 都会记录并授权该网页 Origin；更换网页地址无需清空已保存的 `origins`。业务请求仍必须携带正确 token，`file://` 等不透明来源不支持直接连接，请通过 HTTP(S) 打开画布。
