...
import { ChevronRight, Download, Group, Image as ImageIcon, Music2, Puzzle, RefreshCw, Star, Trash2, Video } from "lucide-react";
...
function mediaAspectRatio(data: CanvasNodeData) {
    if (data.type !== CanvasNodeType.Video) return 0;
    if (data.metadata?.naturalWidth && data.metadata?.naturalHeight) return data.metadata.naturalWidth / data.metadata.naturalHeight;
    const ratio = normalizeVideoRatio(data.metadata?.aspectRatio || data.metadata?.size || "");
    if (!ratio) return 0;
...
type CanvasNodeProps = {
    ...
    onSelectBatchImage?: (nodeId: string, imageId: string) => void;
...
type NodeContentRendererProps = {
    ...
    onSelectBatchImage?: (imageId: string) => void;
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    const handleResizeMouseDown = (event: React.MouseEvent, corner: ResizeCorner) => {
        ...
        resizeRef.current = {
            ...
            ratio: mediaAspectRatio(data) || (data.metadata?.naturalWidth || data.width) / (data.metadata?.naturalHeight || data.height || 1),
        };
    ...
    return (
        <div
            ...
            onMouseDownCapture={(event) => {
                if (event.target instanceof Element && event.target.closest("[data-batch-image-id]")) return;
        ...
        >
            {!referenceSelectionState && (isSelected || (hovered && !selectedBatchImageId) || isEditingTitle) && (
            ...
            >
                <div
                    ...
                    style={
                        {
                            background: isGroup ? "transparent" : hasImageContent || hasVideoContent || transparentBg ? "transparent" : theme.node.fill,
                            pointerEvents: !referenceSelectionState && contentInteractive ? undefined : "none",
                ...
                >
                    <NodeContent
                        ...
                        onSelectBatchImage={(imageId) => onSelectBatchImage?.(data.id, imageId)}
            ...
            {!referenceSelectionState && !isGroup ? <ConnectionHandleDot side="left" visible={(hovered && !selectedBatchImageId) || isSelected || isConnecting} onMouseDown={(event) => onConnectStart(event, data.id, "target")} /> : null}
            {!referenceSelectionState && (definition?.hasSourceHandle ?? true) && data.type !== CanvasNodeType.Config ? <ConnectionHandleDot side="right" visible={(hovered && !selectedBatchImageId) || isSelected || isConnecting} onMouseDown={(event) => onConnectStart(event, data.id, "source")} /> : null}
...
function ImageNodeContent(props: NodeContentRendererProps) {
    ...
    return (
        <ImageContent
            ...
            selectedBatchImageId={props.selectedBatchImageId}
            scale={props.scale}
            onToggleBatch={props.onToggleBatch}
            onSetBatchPrimary={props.onSetBatchPrimary}
            onSelectBatchImage={props.onSelectBatchImage}
        ...
        <BatchFrame batchCount={batchCount} batchExpanded={batchExpanded}>
            {batchExpanded
                ? images
                      ...
                      .map((image, index) => <ExpandedImageCard key={image.id} node={node} image={image} index={index} scale={scale} selected={selectedBatchImageId === image.id} onSelect={() => onSelectBatchImage?.(image.id)} onView={() => onViewBatchImage?.(image.id)} onSetPrimary={() => onSetBatchPrimary?.(image.id)} onRetry={() => onRetryBatchImage?.(image.id)} onDelete={() => onDeleteBatchImage?.(image.id)} />)
                : null}
...
function ExpandedImageCard({ node, image, index, scale, selected, onSelect, onView, onSetPrimary, onRetry, onDelete }: { node: CanvasNodeData; image: CanvasNodeImage; index: number; scale: number; selected: boolean; onSelect: () => void; onView: () => void; onSetPrimary: () => void; onRetry: () => void; onDelete: () => void }) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const { t } = useTranslation();
    useSyncExternalStore(subscribeImagePreviews, getImagePreviewRevision);
    ...
    return (
        <div
            ...
            aria-label={`${node.title} ${index + 1}`}
            className={`group/child absolute z-20 cursor-pointer overflow-hidden rounded-3xl border outline-none ${image.content ? "" : "shadow-[0_18px_50px_rgba(28,25,23,.18)]"}`}
            style={
                {
                    left: x,
                    top: y,
                    ...
                    borderColor: selected ? selectionBlue : image.content ? "transparent" : theme.node.stroke,
                    boxShadow: selected ? `0 0 0 2px ${selectionBlue}66` : undefined,
                    "--batch-from-x": `${-x}px`,
                    "--batch-from-y": `${-y}px`,
                    "--batch-from-rotate": `${4 + index * 2}deg`,
                    animation: `canvas-batch-child-in 320ms ${index * 35}ms cubic-bezier(.2,.85,.18,1) both`,
            ...
            onPointerDown={(event) => {
                event.stopPropagation();
                if (event.button === 0 && !(event.target instanceof Element && event.target.closest("button"))) onSelect();
            }}
        ...
        >
            ...
            {image.content ? (
                <div className="absolute right-2 top-2 opacity-0 transition-opacity duration-150 group-hover/child:opacity-100 group-focus-within/child:opacity-100">
                    <button type="button" className="flex h-8 items-center justify-center gap-1 rounded-lg px-2 text-[10px] font-medium hover:bg-black/5 dark:hover:bg-white/10" style={{ color: theme.toolbar.activeText }} title={t("canvas.node.setPrimary")} onClick={(event) => (event.stopPropagation(), onSetPrimary())}>
