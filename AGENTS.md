...
- OpenAI 视频调用必须按模型隔离协议：仅 `grok-imagine-video*` 使用 grok2api 的 `POST /v1/videos` JSON 字段与后续轮询/下载流程；其他模型保持标准 OpenAI multipart 字段，不得混入 Grok 或其他视频协议参数。
- 模型渠道持久化合并必须区分“无本地配置”和“旧配置缺少 `channels`”：全新环境必须保留完整默认渠道；已有 `channels` 时不得强制补回默认渠道或覆盖用户的删除、自定义结果。
