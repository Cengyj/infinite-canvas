...
- 切换渠道协议只更新协议字段，不得因当前 Base URL 等于预置或官方地址而自动替换地址，也不得重置 API Key、模型或调用脚本。
- 代理只调整请求路由，不得对完整目标 URL 二次解码；拼接 API 路径和流式查询参数时保留原有查询值，修改模型模板不得重写用户已保存的脚本。
- 网站与 API 域名不同是正常直连场景，不得因此要求同域、强制启用代理或判定为跨域失败；代理来源拒绝仅按代理自身诊断标记识别，不按上游错误正文猜测。
- Grok 视频参数只对内置 `grok-imagine-video` 模型启用；普通视频模型必须保留当前首尾帧、多参考图及音视频参考参数，修改模板时先按模型能力分流。
- 合并外部定制时必须对照用户指定的定制目录逐文件核对，不能只依据 VERSION 判断版本差异；用户保存的渠道、脚本、API 地址和请求参数不得被模板更新覆盖。
- 媒体自动清理必须保留生成来源、撤销历史、剪贴板和工作台未提交引用；排队删除在实际执行时再次检查引用，不得用旧快照删除并发新增或重新引用的文件。
