...
Canvas Agent listens on `http://127.0.0.1:17371` by default and generates a connection token. The browser opens an authenticated SSE connection. The EventSource query token is retained temporarily because native EventSource cannot set request headers:
...
Other browser-to-agent REST calls send `X-Canvas-Agent-Token: xxx` instead of putting the token in the URL.
