...
export type InsertAssetPayload = { kind: "text"; content: string; title: string } | { kind: "image"; dataUrl: string; title: string; storageKey?: string } | { kind: "video"; url: string; title: string; storageKey?: string; width?: number; height?: number; aspectRatio?: string; durationMs?: number };
...
function MyAssetsTab({ onInsert }: { onInsert: (payload: InsertAssetPayload) => void }) {
    ...
    const handleInsert = (asset: Asset) => {
        if (asset.kind === "text") {
            ...
            onInsert(asset.kind === "video" ? { kind: "video", url: asset.data.url, storageKey: asset.data.storageKey, title: asset.title, width: asset.data.width, height: asset.data.height, aspectRatio: asset.data.aspectRatio, durationMs: asset.data.durationMs } : { kind: "image", dataUrl: asset.data.dataUrl, storageKey: asset.data.storageKey, title: asset.title });
        }
    };
