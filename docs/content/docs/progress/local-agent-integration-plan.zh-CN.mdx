...
```txt
网页 -> `GET /events?token=xxx&clientId=xxx`（原生 EventSource 暂以 query token 认证）
网页 -> `POST /canvas/state?clientId=xxx`，请求头 `x-canvas-agent-token: xxx`
网页 -> `POST /canvas/result`，请求头 `x-canvas-agent-token: xxx`
```
