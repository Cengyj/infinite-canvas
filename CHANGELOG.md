...
+ [优化] OpenAI 渠道中的 `grok-imagine-video*` 按实际 Grok 接口使用独立 JSON 参数和参考图结构，避免与普通 OpenAI 视频模型字段混发，并复用原有轮询下载流程。
+ [优化] 统一视频任务的轮询、终态解析、鉴权下载与取消清理，并区分视频真实时长和任务生成耗时。
+ [修复] Grok 视频展示区分清晰度、画幅与真实媒体尺寸，竖屏结果不再被错误记录或展示为 `1280×720`。
+ [修复] 画布生成或重试 Grok 视频在缺少媒体像素元数据时按返回画幅调整节点，避免 `9:16` 结果回退为横屏节点。
+ [优化] 画布多图子图支持独立选中，限制成功子图仅可预览或切换为主图，避免误操作整个图片组。

+ [新增] 图片与视频工作台新增各自专用的提示词优化，支持补充要求、结果编辑与复制、多轮优化、生成上下文及确认后回填。
+ [新增] 画布内置图片与视频节点新增画布风格的提示词优化，并按正式生成规则匹配参考图及生成设置。
+ [优化] 提示词优化新增受控参考图视觉分析，按标准多模态协议限制和压缩附件，并保持参考图编号与最终生成请求一致。
+ [修复] 提示词优化在模型切换、请求取消、响应异常及参考图或生成上下文变化时正确失效或阻止错位生成。
+ [修复] 统一 OpenAI 与 Gemini 图片、文本和视频模板的标准字段、接口路径、终态校验及视频内容鉴权下载。
+ [修复] 强化图片与视频工作台参考图上传、数量约束、生成日志和本地清理的并发安全，并拒绝损坏图片及回收失败写入。
+ [修复] 修复提示词优化与全局设置弹层优先级、空模型弹层遮挡及浏览器剪贴板失败误报成功的问题。
+ [调整] 新安装默认 OpenAI 渠道的文本模型由 `gpt-5.5` 更新为 `gpt-5.6-sol`。
