export default {
    grokVideo: { mediaReferenceUnsupported: "Grok video does not support reference videos or audio. Remove them and try again.", durationUnsupported: "Grok video duration must be between 1 and 15 seconds", aspectRatioUnsupported: "Grok video aspect ratio must be 1:1, 16:9, 9:16, 4:3, 3:4, 3:2, or 2:3", resolutionUnsupported: "Grok video quality must be 480p, 720p, or 1080p", resolutionCombinationUnsupported: "Grok 1080p quality requires a 1.5 model and no more than one reference image" },
    meta: {
    ...
    settingsPanels: {
        ...
        video: { title: "Video settings", quality: "Quality", size: "Size", seconds: "Seconds", mode: "Mode", modes: { frames: "First & last frame", reference: "General reference" }, resolution: "Quality tier", ratio: "Aspect ratio", duration: "Duration", smart: "Smart", output: "Output", generateAudio: "Generate audio", watermark: "Add watermark", adaptive: "Adaptive", sizes: { landscape: "Landscape", portrait: "Portrait", square: "Square", widescreen: "Widescreen", tall: "Tall", auto: "Auto" }, ratios: { landscape: "Landscape", portrait: "Portrait", square: "Square", standardLandscape: "Standard landscape", standardPortrait: "Standard portrait", cinematic: "Cinematic", adaptive: "Adaptive" } },
    ...
    modelPlugin: {
        ...
        variables: { prompt: "User prompt with the system prompt already included", images: "Reference image URL array (data URLs or URLs), available for image editing, image-to-video, and multimodal text requests", videos: "Reference videos as a File array you can form.append; empty when none are attached from the canvas or workbench", audios: "Reference audio as a File array you can form.append; empty when none are attached from the canvas or workbench", messages: "Conversation message array including the system message", params: "Generation parameters: image {size,requestedSize,quality,count,background} (requestedSize is the original user size for Gemini only), video {mode,seconds,size,resolution,ratio,generateAudio,watermark} (mode is frames for first/last frame or reference for general references; more than 2 images become reference), audio {voice,format,speed,instructions}", model: "Model name without the provider prefix", baseUrl: "Provider endpoint as entered, without appending /v1", apiKey: "Provider API key; add it to request headers yourself", systemPrompt: "Original system prompt", reasoningEffort: "Text reasoning effort; auto lets the script decide whether to send it", http: "Convenience client: http.post(path, body, {headers,params,responseType}), http.get(path, opts), and http.url(path). Authorization: Bearer apiKey is included by default and can be overridden. Ordinary relative paths append /v1; explicit /v1 and /v1beta paths preserve that version.", request: "Raw request({ method, url, headers, params, data, responseType }) with no default headers. Add authentication yourself. Relative URLs use the same version-path rules as http; full URLs are also accepted.", poll: "poll(request, extract, {intervalMs,timeoutMs}) continues until extract returns a truthy value", sleep: "Delay with sleep(ms)", signal: "Cancellation signal that can be passed to http/request", onDelta: "Push streaming text with onDelta(text) for text models", fetch: "Proxy-aware fetch; HTTP(S) URLs use the configured local proxy and cross-origin or proxy network errors become readable messages" },
        ...
        templates: {
            openaiStandard: "OpenAI Sora (standard fields)",
            geminiVeo3ResolutionUnsupported: "The Veo 3 template currently supports only 720p",
            geminiVeo3SecondsUnsupported: "Veo 3 supports only 4, 6, or 8 seconds",
            geminiVeo2SecondsUnsupported: "Veo 2 supports only 5, 6, or 8 seconds",
            geminiVideoSquareUnsupported: "Gemini Veo does not support square video; choose landscape or portrait",
            videoSingleReference: "The Gemini video template supports at most one reference image",
            openaiVideoSizeUnsupported: "OpenAI video size must be 720x1280, 1280x720, 1024x1792, or 1792x1024",
            openaiVideoSecondsUnsupported: "OpenAI video supports only 4, 8, or 12 seconds",
            openaiVideoSingleReference: "The OpenAI video template supports at most one reference image", openai: "OpenAI format", gemini: "Gemini format" },
        authoring: {
    ...
    imageWorkbench: {
        promptOptimization: {
            ...
            description: "Use a text model to clarify the visual direction. The result is only written back after you confirm it and never starts generation automatically.",
            model: "Optimization model",
            referenceCount: "There are {{count}} reference images. For privacy, image content is not analyzed when the dialog first opens.",
            noReferences: "There are no reference images; the optimization request sends only the prompt and generation context.",
            analyzeReferences: "Analyze reference images",
            referenceLimit: "Up to {{count}} reference images can be analyzed at once. Remove some images or leave reference-image analysis off.",
            analyzeReferencesHint: "When enabled, up to {{count}} reference images are compressed and sent to the current text model so it can use visible details when optimizing the prompt.",
            referencesTextOnly: "While the switch is off, image content is not sent; only the labels and count for the {{count}} reference images, plus the current generation context, are included.",
            source: "Current prompt",
            sourceHint: "Editable here",
            sourcePlaceholder: "Enter an image prompt to optimize",
            requirements: "Additional requirements",
            requirementsHint: "Optional",
            requirementsPlaceholder: "For example: preserve negative space, use a top-down view, avoid visible text",
            requirementsReferencePlaceholder: "For example: preserve the subject from Image 1 and use Image 2 only for clothing or style",
            result: "Optimized result",
            resultHint: "Editable before applying",
            resultPlaceholder: "The optimized result will appear here",
            optimizing: "Generating the optimized result…",
            optimize: "Optimize",
            reoptimize: "Optimize again",
            continue: "Continue from result",
            stop: "Stop optimization",
            apply: "Apply to prompt",
            applied: "Applied the optimized prompt",
            referenceChanged: "Reference images changed. Check the image labels in the prompt, then generate again.",
            contextChanged: "Frame size or transparent-background settings changed. Confirm the optimized prompt still applies, then generate again.",
            referenceAndContextChanged: "Reference images and frame settings changed. Check image labels and confirm the optimized prompt still applies, then generate again.",
            configRequired: "Configure an available text model and provider credentials first",
            failed: "Prompt optimization failed",
            status: { waiting: "Not optimized", running: "Optimizing", ready: "Ready to apply", stale: "Optimize again" },
        },
        editReferenceLimit: "OpenAI-format image editing supports up to {{count}} reference images. Remove some images.",
        promptActions: { prompts: "Prompts", assets: "Assets", optimize: "Optimize" },
        title: "Image Studio",
    ...
    videoWorkbench: {
        promptOptimization: {
            action: "Optimize prompt",
            title: "Optimize video prompt",
            description: "Use a text model to improve action, camera, and timing. The result is only written back after you confirm it and never starts video generation automatically.",
            model: "Optimization model",
            referenceCount: "There are {{count}} reference images. For privacy, image content is not analyzed when the dialog first opens.",
            noReferences: "There are no reference images; the optimization request sends only the prompt and generation context.",
            analyzeReferences: "Analyze reference images",
            referenceLimit: "Up to {{count}} reference images can be analyzed at once. Remove some images or leave reference-image analysis off.",
            analyzeReferencesHint: "When enabled, up to {{count}} reference images are compressed and sent to the current text model to preserve subject, scene, and style continuity.",
            referencesTextOnly: "While the switch is off, image content is not sent; only the labels and count for the {{count}} reference images, plus the current generation context, are included.",
            source: "Current prompt",
            sourceHint: "Editable here",
            sourcePlaceholder: "Enter a video prompt to optimize",
            requirements: "Additional requirements",
            requirementsHint: "Optional",
            requirementsPlaceholder: "For example: use a slow push-in, keep the subject consistent, and end on a wide shot",
            requirementsReferencePlaceholder: "For example: use Image 1 for the subject, Image 2 only for the scene, and add a slow push-in",
            result: "Optimized result",
            resultHint: "Editable before applying",
            resultPlaceholder: "The optimized result will appear here",
            optimizing: "Generating the optimized result…",
            ...
            stop: "Stop optimization",
            apply: "Apply to prompt",
            applied: "Applied the optimized video prompt",
            referenceChanged: "Reference images changed. Check the image labels in the prompt, then generate again.",
            contextChanged: "Video duration, frame size, or audio settings changed. Confirm the optimized prompt still applies, then generate again.",
            referenceAndContextChanged: "Reference images and video settings changed. Check image labels and confirm the optimized prompt still applies, then generate again.",
            configRequired: "Configure an available text model and provider credentials first",
            failed: "Prompt optimization failed",
            status: { waiting: "Not optimized", running: "Optimizing", ready: "Ready to apply", stale: "Optimize again" },
        },
        ...
        dropReferences: "Drop to add reference images",
        noImages: "No reference images. Drag in up to {{count}} images.",
        noImagesUnlimited: "No reference images yet. Drag images here.",
        ...
        unsupportedFiles: "Non-image files were ignored. Video Studio accepts images only as reference input.",
    ...
    config: {
        ...
        proxy: {
            title: "Local proxy",
            description: "Optional forwarding tool. Your website and API can use different domains without enabling a proxy. Enable it when browser CORS policy blocks a direct request; model requests, resource downloads, and WebDAV sync will then use the local proxy.",
            startHint: "Run this in a terminal first, and keep it running while you use the canvas:",
            originHint: "The proxy allows valid HTTP(S) page origins by default. To tighten it, add --allow-origin <page Origin>; repeat the option for multiple pages.",
            ...
            originNotAllowed: "The local proxy rejected this page origin. For HTTP(S) pages, remove an overly narrow --allow-origin setting or add this Origin; open file:// pages over HTTP(S) instead.",
        },
        localStorage: {
        ...
        modelSelect: {
            ...
            description: "If the provider does not expose a model-list endpoint, add model names manually here.",
        ...
        webdav: {
            localReadFailed: "Local data could not be fully loaded. Sync was stopped to protect existing files; reload and try again.",
            title: "WebDAV sync",
